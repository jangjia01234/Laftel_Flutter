...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      bottomNavigationBar: const BottomAppBar(child: BottomTabMenuBar()),
    ));
  }
}
    ...
    return SingleChildScrollView(
        ...
        child: Row(
          children: [
            ...
            Column(
              children: [
                hotSingleVideoThumbnailCard("4", "전생했더니 슬라임이었던 건에 대하여", "이세계/판타지"),
                SizedBox(height: 10),
                hotSingleVideoThumbnailCard("5", "(더빙) 꿈빛 파티시엘 리마스터", "로맨스/음식"),
                SizedBox(height: 10),
                hotSingleVideoThumbnailCard("6", "(자막) 귀멸의 칼날: 환락의 거리편", "판타지/액션"),
              ],
            ),
            Column(
              children: [
      ...
      Row(
        children: [
          Padding(
            ...
            child: Text(rank, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          ),
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
...
// MARM: 하단 메뉴 탭바
class BottomTabMenuBar extends StatelessWidget {
  const BottomTabMenuBar({super.key});
